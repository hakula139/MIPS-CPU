...
本来 `ihit`、`dhit`、`dcen` 还要用于决定是否 stall 流水线，但这里直接通过屏蔽 CPU 时钟实现了，就无需另外改动 mips 的代码（虽然我已经改了，不删了）。
...
|:------:|:--------:|:--------:|:--------:|
| LRU    | 2.060085 | 2.059883 | 2.067166 |
| RR     | 2.052802 | 2.058062 | 2.067166 |
| LFU    | 2.078293 | 2.079911 | 2.067166 |

1. 256 Bytes 4 路组相联映射（默认）：$(t,s,b,e) = (26,2,4,4)$
2. 256 Bytes 2 路组相联映射：$(t,s,b,e) = (25,3,4,2)$
3. 256 Bytes 全相联映射：$(t,s,b,e) = (24,4,4,1)$

可见在缓存大小不变的情况下，缓存效果大体上呈现 RR ≈ LRU > LFU 的趋势（全相联映射下替换策略无意义，在此忽略），而路数的影响似乎并不大。当然这也和测试样例的设计有关，这里样本本身也很少，只是作一个粗略的估计。

如果将缓存扩大一倍，同时相应地路数也增加一倍呢？

| Policy | CPI #1   | CPI #2   | CPI #3   | CPI #4   |
|:------:|:--------:|:--------:|:--------:|:--------:|
| LRU    | 1.855958 | 1.853935 | 1.853935 | 1.853935 |
| RR     | 1.855958 | 1.853935 | 1.853935 | 1.853935 |
| LFU    | 1.855958 | 1.853935 | 1.853935 | 1.853935 |

1. 512 Bytes 8 路组相联映射：$(t,s,b,e) = (26,2,4,8)$
2. 512 Bytes 4 路组相联映射：$(t,s,b,e) = (25,3,4,4)$
3. 512 Bytes 2 路组相联映射：$(t,s,b,e) = (24,4,4,2)$
4. 512 Bytes 全相联映射：$(t,s,b,e) = (23,5,4,1)$
...
| Policy | CPI #1   | CPI #2   |
|:------:|:--------:|:--------:|
| LRU    | 2.197731 | 2.179960 |
| RR     | 2.179477 | 2.179960 |
| LFU    | 2.180603 | 2.179960 |

1. 128 Bytes 2 路组相联映射：$(t,s,b,e) = (26,2,4,2)$
2. 128 Bytes 全相联映射：$(t,s,b,e) = (25,3,4,1)$
